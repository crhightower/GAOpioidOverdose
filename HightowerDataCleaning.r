...
#Percentage of population that is non-Hispanic White (Previous S2702_C01_026)
...
ACS <- get_acs(geography = "county",
               variables = c(pctuninsured = "S2701_C05_001", totvehicle = "B08141_001", novehicle = "B08141_002",
                             ...
                             pctnonhiswhite = "DP05_0077P", pctunemployed = "S2301_C04_001", teenbirth = "S1301_C04_002",
...
Buprenorphine <- Buprenorphine %>%
                 ...
                 select(county, n) %>%
                 distinct() %>%
                 rename(County = county, BuprenorphineProviders = n) %>%
...
possible.pred4$BuprenorphineProviders[is.na(possible.pred4$BuprenorphineProviders)] <- 0

#create rate
possible.pred4 <- possible.pred4 %>% mutate(BupProviderspercap = BuprenorphineProviders / population)
possible.pred4$BupProviderspercap[is.na(possible.pred4$BupProviderspercap)] <- 0
...
#All drugs
#issues with numbers with commas. e.g. 1,308
alldrug.nonfatal <- read.csv(file = 'ERDrugOverdoses.csv', header = T) 

alldrug.nonfatal <- alldrug.nonfatal %>%
                    rename(eralldrug = ER.Inpatient.Visits, eralldrugR = ER.Inpatient.Visit.Rate) %>%
                    na_if("*") %>%
                    mutate(eralldrug = as.numeric(gsub(",", "", eralldrug)), eralldrugR = as.numeric(eralldrugR))
...
#note cases <5 are hidden for counties, so we are going to impute these with 2.5
chlamydia.gonorrhea <- read.csv(file = 'ChlamydiaGonorrhea.csv', header = T) %>%
                       ...
                       mutate_at("std",~replace(., is.na(.), 2.5))
...
syphilis <- read.csv(file = 'Syphilis.csv', header = T) %>%
            ...
            mutate_at("syphilis",~replace(., is.na(.), 2.5))
...
#<5 is censored with -1, so impute with 2.5
HIV.case2 <- HIV.case %>% filter(substr(GEO.ID,1,2) == "13") %>%
                          select(GEO.ID, County.Name, New.Diagnoses.Rate, New.Diagnoses.Cases) %>%
                          rename(County = County.Name, FIPS = GEO.ID, HIVnewcases = New.Diagnoses.Cases, HIVincidence = New.Diagnoses.Rate) %>%
                          mutate_at("HIVnewcases",~replace(., . == -1, 2.5)) %>%
                          mutate(HIVincidence = replace(HIVincidence, which(HIVincidence<0), NA),
                                 HIVnewcases = replace(HIVnewcases, which(HIVnewcases<0), NA)) #impute negatives
...
#replace -1 with 3 because they censor <5
HIV.prev2 <- HIV.prev %>% filter(State.Abbreviation == "GA") %>%
                          select(GEO.ID, County.Name, County.Cases, County.Rate, IDU.Cases) %>%
                          rename(County = County.Name, FIPS = GEO.ID, HIVprevalence = County.Cases, HIVprevalenceR = County.Rate, HIV_IDU = IDU.Cases) %>%
                          mutate_at(c("HIVprevalence", "HIV_IDU"),~replace(., . == -1, 2.5)) %>%
                          mutate(HIVprevalenceR = replace(HIVprevalenceR, which(HIVprevalenceR<0), NA), 
                                 HIVprevalence = replace(HIVprevalence, which(HIVprevalence<0), NA),
...
OpioidOverlap2 <- OpioidOverlap %>% filter(row_number() %% 3 != 1) %>%
                                    ...
                                    mutate(overlap.opioid = as.numeric(overlap.opioid),
                                           overlap.benzo = as.numeric(overlap.benzo))
...
#I can't rerun it so I have to let the code be as is


###################################################

#read in the data because I can't rerun it
setwd("H:/chight3/SVI/")
NPI <- read.csv("NPI_finalgeocode.csv", header = T)

#investigate misnamed counties, and set counties geocoded as outside GA as missing
NPI$County[NPI$County == "De Kalb"] <- "DeKalb"
NPI$County[NPI$County == "Mcintosh"] <- "McIntosh"
NPI$County[NPI$County == "Mcduffie"] <- "McDuffie"

length(table(NPI$County)) #192
table(NPI$County)

NPI <- NPI %>% mutate(County = paste0(County, " County, Georgia")) 

bad.counties <- outersect(unique(NPI$County), possible.pred10$County)

NPI <- NPI %>% mutate(County = ifelse(County %in% bad.counties, NA, County))
...
#get only the codes
mentalhealth.tax <- mentalhealth.tax %>% mutate(mh = sapply(strsplit(V1, " "),`[`,1), V1 = NULL)

#count number of mental health provider in each County
MentalHealth <- NPI %>% filter(Healthcare.Provider.Taxonomy.Code_1 %in% mentalhealth.tax$mh) %>%
                        add_count(County) %>%
                        select(County, n) %>%
                        distinct() %>%
                        rename(MentalHealthProviders = n) %>%
                        filter(!is.na(County)) #remove counties that weren't geocoded

#Primary Care taxonomy codes
PrimaryCare.tax <- as.data.frame(primary <- c("207Q00000X",
                                              "261QF0400X",
                                              "208D00000X",
                                              "207R00000X",
                                              "363L00000X",
...
#count number of mental health provider in each County
PrimaryCare <- NPI %>% filter(Healthcare.Provider.Taxonomy.Code_1 %in% PrimaryCare.tax$primary) %>%
                       add_count(County) %>%
                       select(County, n) %>%
                       distinct() %>%
                       rename(PrimaryCareProviders = n) %>%
                       filter(!is.na(County)) #remove counties that weren't geocoded

#SpecialtyProviders
#that is hepatology, gastroentology, and infectious diseases
#not including pediatric
SpecialtyCare.tax <- as.data.frame(specialty <- c("207RI0200X",  #infectious disease
                                                  "207RG0100X",  #gastroentology
                                                  "207RI0008X")) #hepatology 

SpecialtyCare <- NPI %>% filter(Healthcare.Provider.Taxonomy.Code_1 %in% SpecialtyCare.tax$specialty) %>%
                         add_count(County) %>%
                         select(County, n) %>%
                         distinct() %>%
                         rename(SpecialtyCareProviders = n) %>%
                         filter(!is.na(County)) #remove counties that weren't geocoded
                                                  
#Urgent Care facilities
UrgentCare.tax <- as.data.frame(urgent <- c("261QU0200X"))

UrgentCare <- NPI %>% filter(Healthcare.Provider.Taxonomy.Code_1 %in% UrgentCare.tax$urgent) %>%
                      add_count(County) %>%
                      select(County, n) %>%
                      distinct() %>%
                      rename(UrgentCareProviders = n) %>%
                      filter(!is.na(County)) #remove counties that weren't geocoded

#Substance use providers
SubstanceUse.tax <- as.data.frame(substance <- c("261QR0800X",
                                                 "324500000X"))

SubstanceUseProviders <- NPI %>% filter(Healthcare.Provider.Taxonomy.Code_1 %in% SubstanceUse.tax$substance) %>%
                                 add_count(County) %>%
                                 select(County, n) %>%
                                 distinct() %>%
                                 rename(SubstanceUseProviders = n) %>%
                                 filter(!is.na(County)) #remove counties that weren't geocoded

#when I merge these with the other possible predictors, I want rates and to replace NAs with 0.

#merge provider information together and set NAs to zero (since we have all -most- the information for the counties)
possible.pred11 <- possible.pred10 %>% full_join(MentalHealth, by = "County") %>%
                                       full_join(PrimaryCare, by = "County") %>%
                                       full_join(SpecialtyCare, by = "County") %>%
                                       full_join(UrgentCare, by = "County") %>%
                                       full_join(SubstanceUseProviders, by = "County") %>%
                                       mutate_at(c("MentalHealthProviders",
                                                   "PrimaryCareProviders",
                                                   "SpecialtyCareProviders",
                                                   "UrgentCareProviders",
                                                   "SubstanceUseProviders",
                                                   "BuprenorphineProviders") ,~replace(., is.na(.), 0))
#merge good, zeros good

#If I have time, create some sort of highway access variable in ArcMap

#If I have time, I can also geocode opioid treatment programs from SAMHSA

#Neonatal Abstinence Syndrome data
#* Counts not displayed for counties with 1-9 NAS cases. 
#** NAS rates not calculated for counties with 1-9 cases of NAS.
NAS <- read.csv("NAS.csv", header = T)

head(NAS)

#calculate the rate with live births or population?
#impute censored with 5
NAS2 <- NAS %>% select(-Rate.of.NAS, -Number.of.Live.Births) %>%
                rename(nas = Number.of.NAS.Cases, urbanrural = NCHS.Urban.Rural.Status) %>%
                na_if("*") %>%
                mutate(nas = as.numeric(nas), urbanrural = as.factor(urbanrural), County = paste0(County, " County, Georgia")) %>%
                mutate_at("nas",~replace(., is.na(.), 5))

head(NAS2)

#merge with possible.pred11
possible.pred12 <- possible.pred11 %>% select(-urbanrural) %>%
                                       full_join(NAS2, by = "County")
...
#Recreate rates for all drugs and stds (note a lot of these are unstable)
#Create rates for providers
#create a violent crimes variable
#and property crimes variable
possible.pred13 <- possible.pred12 %>% mutate(deathalldrugR = deathalldrug / population*100000,
                                              deathallopioidR = deathallopioid / population*100000,
                                              deathheroinR = deathheroin / population*100000,
                                              deathsyntheticR = deathsynthetic / population*100000,
                                              deathnaturalR = deathnatural / population*100000,
                                              deathmethadoneR = deathmethadone / population*100000,
                                              stdR = std / population*100000,
                                              syphilisR = syphilis / population*100000,
                                              eralldrugR = eralldrug / population*100000,
                                              erallopioidR = erallopioid / population*100000,
                                              erheroinR = erheroin / population*100000,
                                              HIVincidence = HIVnewcases / population*100000,
                                              HIVprevalenceR = HIVprevalence / population*100000,
                                              HIV_IDUR = HIV_IDU / population*100000,
                                              prepR = prep / population*100000,
                                              MentalHealthR = MentalHealthProviders / population*100000,
                                              PrimaryCareR = PrimaryCareProviders / population*100000,
                                              SpecialtyCareR = SpecialtyCareProviders / population*100000,
                                              UrgentCareR = UrgentCareProviders / population*100000,
                                              SubstanceUseR = SubstanceUseProviders / population*100000,
                                              BupProviderspercap = BupProviderspercap*100000,
                                              violentcrimes = Murder + Rape + Robbery + Assault,
                                              propertycrimes = Burglary + Larceny + Vehicle.Theft,
                                              Murder = NULL, Rape = NULL, Robbery = NULL, Assault = NULL, Burglary = NULL, Larceny = NULL, Vehicle.Theft = NULL)
